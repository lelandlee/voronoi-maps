const demoCoords = [103.89623,1.394897]
...
voronoiMap = function(map, url, initialSelections) {
  ...
  var selectPoint = function() {
    ...
    d3.select('#selected h1')
      .html('')
      .append('a')
        .text(point.name + ' - ' + point.zipCode + ' - [' + point.longitude + ',' + point.latitude + ']')
        .attr('target', '_blank')
  ...
  function getLocation(){
    navigator.geolocation.getCurrentPosition(function(geoposition){
      ...
      var locLayer = L.mapbox.featureLayer({
        ...
        geometry: {
          type: 'Point',
          coordinates: demoCoords//[latitude, longitude]
        },
        properties: {
      ...
      locLayer.on('click', function(e){
        console.log('clicked')
        map.setView([1.394897, 103.89623], 17)

        console.log(e.layer.feature, locLayer.getGeoJSON())
        //var mapFeatures = toGeoJSON(points)

        //var nearestMapFeature = turf.nearest(e.layer.feature, mapFeatures);

        //var id = nearestMapFeature.properties['id'];
        //console.log(nearestMapFeature, points[id], id)
        //points[id].r = 100
        //draw()
